...
-- 发文
          insert into TEMP_NUMBER50
          (select id,subject,doc_mark,issuer,send_department,pack_date,val,create_time,year,edoc_type,org from (
            select A.id,subject,doc_mark, issuer,A.name send_department,A.pack_date, 0 val,
            ...
            from
          -- edoc_summary
          (select  es.* from  (select substr(t.SUBJECT,5,2) name,s.* from CTP_TEMPLATE t , (select *  from EDOC_SUMMARY where EDOC_TYPE=0 and HAS_ARCHIVE=1) s where t.id=s.TEMPLETE_ID)  es,
          ...
          and a.EDOC_TYPE = 0 and a.name in ('党办','院办','纪委')
          ) c );
...
--           收文
            ...
            from
          -- edoc_summary
          (select  es.* from (select * from  EDOC_SUMMARY where has_archive = 1 and EDOC_TYPE =1) es,
          (select * from temp_number40 where content_type in (41,42,43,44,45) and  content is not null) con
          where con.module_id=es.id )
          A, (
              select zall.*,CF.MIME_TYPE,CF.id from (select to_number(content) content,MODULE_ID from temp_number40 where to_char(content)
...
--           签报
            ...
            (select id,subject,doc_mark,issuer,send_department,pack_date,val,create_time,year,edoc_type,org from (
                ...
                from
              -- edoc_summary
              (select  es.* from (select * from  EDOC_SUMMARY where has_archive = 1 and EDOC_TYPE =2) es,
              (select * from temp_number40 where content_type in (41,42,43,44,45) and  content is not null) con
              where con.module_id=es.id )
              A, (
                  select zall.*,CF.MIME_TYPE,CF.id from (select to_number(content) content,MODULE_ID from temp_number40 where to_char(content)
        ...
        begin
              --发文
                ...
                (select * from TEMP_NUMBER50 n50 where  exists (select * from temp_number10 t where status='0' and n50.id= t.id) and n50.edoc_type='发文');

              --收文
              insert into TEMP_NUMBER20
                (select * from TEMP_NUMBER50 n50 where  exists (select * from temp_number10 t where status='0' and n50.id= t.id) and n50.edoc_type='收文');

              --签报
              insert into TEMP_NUMBER20
                 (select * from TEMP_NUMBER50 n50 where  exists (select * from temp_number10 t where status='0' and n50.id= t.id) and n50.edoc_type='签报');
        exception
            when others then
                ROLLBACK;
