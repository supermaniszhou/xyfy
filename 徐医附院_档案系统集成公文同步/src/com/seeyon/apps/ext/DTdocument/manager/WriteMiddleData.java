    ...
    public void getListData(String sql, String type) {
        ...
        try {
            ...
            if (listTemp.size() > 0) {
                ...
                String updateToa="update T_OA set c3=? where id=?";
                PreparedStatement psToa=mConn.prepareStatement(updateToa);
                for (int i = 0; i < listTemp.size(); i++) {
                    if (toaId.size() > 0) {
                        if (!toaId.contains(listTemp.get(i).getId())) {
                            ...
                            psToa.setString(1,listTemp.get(i).getSend_departmen());
                            psToa.setString(2,listTemp.get(i).getId());
                            psToa.addBatch();
                        }
                    } else {
                        mPs.setString(1, listTemp.get(i).getId());
