...
select A.id,subject,doc_mark, issuer,A.name send_department,A.pack_date, 0 val,to_char(a.create_time,'yyyyMMdd') create_time,to_char(a.create_time,'yyyy') year,  CASE A .EDOC_TYPE WHEN 0 THEN  '发文' WHEN 1 THEN  '收文' ELSE  '签报' END EDOC_TYPE,'' org
from (select substr(t.SUBJECT,5,2) name,s.* from CTP_TEMPLATE t RIGHT JOIN EDOC_SUMMARY s on t.id=s.TEMPLETE_ID ) A, (
SELECT  zall.* FROM  (select * from TEMP_NUMBER40 where CONTENT_TYPE in (10,30) ) zall
) b where A.has_archive = 1
and a.id = b.MODULE_ID
 and a.EDOC_TYPE =0 and a.name in ('党办','院办','纪委')) cd where  exists(select * from temp_number10 t where t.status='0' and cd.id=t.id);
